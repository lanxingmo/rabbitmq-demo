...
const MQURL ="amqp://admin:admin@172.16.0.149:5672/"  // "amqp://imoocuser:imoocuser@127.0.0.1:5672/imooc"
...
//订阅模式生成，要提前绑定队列，才能持久化
func (r *RabbitMQ) PublishPub2(message string) {
	//尝试创建交换机，不存在创建
	err := r.channel.ExchangeDeclare(
		//交换机名称
		r.Exchange,
	...
	fmt.Println(err, "failed to declare an excha" + "nge")
	//2试探性创建队列，创建队列
	q, err := r.channel.QueueDeclare(
		"test4",//随机生产队列名称
		true,
		false,
		false,  //注意此字段要为false才不会丢消息
		false,
		nil,
	)
	q.Name="test4"
